...
class mod_cap_production extends module_abstract {
	...
	{
		global $PDDB;
		$PDDB = new Database("localhost", 'evedb', 'evedb', 'Evedbpw1234');
		$this->template = $template;
		$er = ErrorHandler::getErrorHandler();
		
		$existing_job = request_var('jobid', 0);
		$delete_job = request_var('deletejob', 0);
		if(!empty($existing_job)) {
			$this->job = new production_job($existing_job);
		}
		if(!empty($delete_job) && empty($existing_job)) {
			$sql = "SELECT count(jobID) as cJid FROM emb_product_job WHERE jobID = ".intval($delete_job);
			$xRes = Database::getInstance()->sql_fetch_array(Database::getInstance()->sql_query($sql));
			if($xRes['cJid'] > 0) {
				$tempjbb = new production_job($delete_job);
				$tempjbb->DB_Delete();
			}
		}
		...
		$this->displayMineralPrices();
		$this->displayMineralCount();
	...
	private function displayJobDetails() {
		...
		if($var == 1) {
			...
			$notice = $this->makeSQLSafe($notice);
