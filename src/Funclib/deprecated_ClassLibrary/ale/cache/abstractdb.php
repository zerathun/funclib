<?php
/**
 * @version $Id: abstractdb.php 213 2009-12-15 21:27:13Z kovalikp $
...
abstract class AleCacheAbstractDB implements AleInterfaceCache {
	...
	public function store($content, $cachedUntil) {
		...
		if ($this->row) {
			...
			$query = sprintf ( 'UPDATE %s SET %s = %s, %s = %s WHERE %s', $this->quoteName ( $this->table ), $this->quoteName ( 'content' ), $this->quote ( $content ), $this->quoteName ( 'cachedUntil' ), $cachedUntil, $this->getWhere () );
		} else {
			$this->row = array ();
			$this->row ['content'] = $content;
			$this->row ['cachedUntil'] = $cachedUntil;
			...
			$query = sprintf ( 'INSERT INTO %s (%s) VALUES (%s);', $this->quoteName ( $this->table ), implode ( ', ', $fields ), implode ( ', ', $values ) );
		}
		$this->execute ( $query );
	}
	...
	public function updateCachedUntil($time) {
		if ($this->row) {
			...
			$query = sprintf ( 'UPDATE %s SET %s = %s WHERE %s', $this->quoteName ( $this->table ), $this->quoteName ( 'cachedUntil' ), $cachedUntil, $this->getWhere () );
			$this->execute ( $query );
		}
	}
	...
	public function purge($all = false) {
		if ($all) {
			...
			$tz = new DateTimeZone ( 'UTC' );
			$now = new DateTime ( null, $tz );
			$query = sprintf ( "DELETE FROM %s WHERE %s = %s AND %s < %s", $this->quoteName ( $this->table ), $this->quoteName ( 'host' ), $this->quote ( $this->host ), $this->quoteName ( 'cachedUntil' ), $this->quote ( $now->format ( '' ) ) );
		}
		$this->execute ( $query );
	}
}